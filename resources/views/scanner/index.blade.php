    ...
    <div class="bg-white w-11/12 max-w-md rounded-2xl shadow-xl p-6 relative">
      ...
      <div class="flex justify-end space-x-3 mt-6">
        ...
        <button id="sendWhatsapp"
          class="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg">Send via WhatsApp</button>
      </div>
    </div>
  </div>
    ...
    async function fetchInviteDetails(qrCode) {
      try {
        ...
        if (!response.ok || data.status === 'error') {
          ...
          currentWhatsappUrl = data.whatsapp_url; // save link for Send button

          const content = `
            <p><strong>Name:</strong> ${invite.name}</p>
            <p><strong>Company:</strong> ${invite.company || 'N/A'}</p>
            <p><strong>Invites:</strong> ${invite.number_of_invites}</p>
          ...
          displayResult("Ticket scanned successfully");
        }
      } catch (error) {
